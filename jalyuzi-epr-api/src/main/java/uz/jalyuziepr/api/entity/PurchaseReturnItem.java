...
@AllArgsConstructor
@Builder
public class PurchaseReturnItem extends BaseEntity implements Auditable {
    ...
    public Map<String, Object> toAuditMap() {
        ...
        map.put("id", getId());
        map.put("returnedQuantity", this.returnedQuantity);
        map.put("unitPrice", this.unitPrice);
        map.put("totalPrice", this.totalPrice);

        // Avoid lazy loading
        if (this.purchaseReturn != null) {
            map.put("purchaseReturnId", this.purchaseReturn.getId());
        }
